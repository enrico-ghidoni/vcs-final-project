    ...
    while cap.isOpened():
        ...
        if ret:
            if frame_count % onein == 0:
                ...
                print(f'Frame {frame_count}, found {len(contours)} contours')
                ...
                # salvo i bounding boxes nel buffer da scrivere su file
                for box in boundRect:
                    x, y, w, h = box[0], box[1], box[2], box[3]
                    if not frame_count in bboxes_json:
                        bboxes_json[frame_count] = []
                    bboxes_json[frame_count].append((x, y, w, h))
        else:
            break
